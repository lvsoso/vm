...
/* OpArgMask */
const (
	OpArgN = iota // argument is not used 操作数不表示任何信息
	OpArgU        // argument is used
	OpArgR        // argument is a register or a jump offset 操作数在iABC模式下表示寄存器索引，在iAsBx模式下表示跳转偏移
	OpArgK        // argument is a constant or register/constant 操作数表示常量表索引或者寄存器索引
)
